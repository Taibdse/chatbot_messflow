        ...
        var buildSubUL = function (parent, items) {
            $.each(items, function () {
                if (this.label) {
                    ...
                    var li = $("<li><a href='" + this.url + "'><i class='fa fa-circle-o'></i> " + this.label + "</a></li>");
                    li.appendTo(parent);
                }
            });
        }
    ...
    Array.from($(".sidebar-menu").find('a')).forEach(a => {
        if ($(a).attr('href') == location.pathname) {
            if ($(a).parent('li').parent('ul').hasClass('sidebar-menu')) {
                ...
                $(a).parent('li').parents('li').find('.fa.fa-angle-left').removeClass('fa-angle-left').addClass('fa-angle-down');
